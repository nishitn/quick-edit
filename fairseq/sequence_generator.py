...
class SequenceGenerator(object):
    ...
    def _generate(self, src_tokens, src_lengths, guess_tokens, guess_lengths, marker, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        bsz2, guesslen = guess_tokens.size() #------------------------------------------------------------------------------------
        maxlen = min(maxlen, self.maxlen) if maxlen is not None else self.maxlen
        # print(src_tokens, ' ', guess_tokens)
        ...
        for model in self.models:
            ...
            guess_encoder_out = model.guess_encoder(
                ...
                guess_lengths.repeat(beam_size), marker #--------------------------
            )
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            probs, avg_attn_scores = self._decode(
                tokens[:, :step+1], encoder_outs, guess_encoder_outs, marker, incremental_states) #----------------------------------
    ...
    def _decode(self, tokens, encoder_outs, guess_encoder_outs, marker, incremental_states): #-----------------------------------------------
        # wrap in Variable
