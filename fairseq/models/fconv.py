...
class FConvModel(FairseqModel):
    def __init__(self, encoder, guess_encoder, decoder):
        super().__init__(encoder, guess_encoder, decoder)
        self.encoder.num_attention_layers = sum(layer is not None for layer in decoder.attention)
        self.guess_encoder.num_attention_layers = sum(layer is not None for layer in decoder.attention)
    ...
    def add_args(parser):
        ...
        #------------------------------------------------------------------------------------------------------------------ 
        parser.add_argument('--guess-encoder-embed-dim', type=int, metavar='N',
                            help='guess encoder embedding dimension')
        parser.add_argument('--guess-encoder-embed-path', default=None, type=str, metavar='STR',
                            help='path to pre-trained guess encoder embedding')
        parser.add_argument('--guess-encoder-layers', type=str, metavar='EXPR',
                            help='guess encoder layers [(dim, kernel_size), ...]')
    ...
    def build_model(cls, args, src_dict, dst_dict):
        ...
        return FConvModel(encoder, guess_encoder, decoder) #---------------------------------------------------------
...
#--------------------------------------------------------------------------------------------------------------------
class FConvGuessEncoder(FairseqGuessEncoder):
    """Convolutional encoder"""
    def __init__(self, dictionary, embed_dim=512, embed_dict=None, max_positions=1024, convolutions=((512, 3),) * 20, dropout=0.1):
        super().__init__(dictionary)
        self.dropout = dropout
    ...
    def forward(self, guess_tokens, guess_lengths, marker):
        ...
        # add output to input embedding for attention
        y = (x + guess_embedding) * math.sqrt(0.5) #-----------------------------------------------------------------------
...
class FConvDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out, guess_encoder_out, incremental_state=None): #----------------------------
        # split and transpose encoder outputs
        encoder_a, encoder_b = self._split_encoder_out(encoder_out, incremental_state)
        guess_encoder_a, guess_encoder_b = self._split_encoder_out(guess_encoder_out, incremental_state) #----------------------
        ...
        #print(x.shape, avg_attn_scores1.shape, avg_attn_scores2.shape, ' b')
...
def fconv_iwslt_de_en(args):
    ...
    args.guess_encoder_embed_dim = 256
    args.guess_encoder_layers = '[(256, 3)] * 4'
